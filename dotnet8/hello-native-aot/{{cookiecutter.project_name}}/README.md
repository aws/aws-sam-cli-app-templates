...
```bash
{{ cookiecutter.project_name }}$ sam local invoke HelloWorldAotFunction --event events/event.json
```
...
```bash
{{ cookiecutter.project_name }}$ sam logs -n HelloWorldAotFunction --stack-name {{ cookiecutter.project_name }} --tail
```
...
```bash
{{ cookiecutter.project_name }}$ dotnet test test/HelloWorldAot.Test
```

...
```bash
{{cookiecutter.project_name}}$ sam local invoke ScheduledEventLogger --event events/event-cloudwatch-event.json
```
...
Resources:
  ScheduledEventLogger:
    ...
    Properties:
      FunctionName: ScheduledEventLoggerFunction
...
```bash
{{cookiecutter.project_name}}$ sam logs -n ScheduledEventLogger --stack-name sam-app --tail
```

...
- {{cookiecutter.function_name}}/src/main - Code for the application's Lambda function.
- events - Invocation events that you can use to invoke the function.
- {{cookiecutter.function_name}}/src/test - Unit tests for the application code. 
...
The SAM CLI installs dependencies defined in `{{cookiecutter.function_name}}/build.gradle`, creates a deployment package, and saves it in the `.aws-sam/build` folder.

Test a single function by invoking it directly with a test event. An event is a JSON document that represents the input that the function receives from the event source. Test events are included in the `events` folder in this project.

Run functions locally and invoke them with the `sam local invoke` command.

Don't forget to update the event.json detail with the fields you want to set from your {{ cookiecutter.AWS_Schema_root }}.{{ cookiecutter.AWS_Schema_name }} object
...
Tests are defined in the `{{cookiecutter.function_name}}/src/test` folder in this project.

```bash
{{ cookiecutter.project_name }}$ cd {{cookiecutter.function_name}}
{{cookiecutter.function_name}}$ gradle test
```

...
```bash
{{cookiecutter.project_name}}$ sam local invoke SQSPayloadLogger --event events/event-sns.json
```
...
```bash
{{cookiecutter.project_name}}$ sam logs -n SNSPayloadLogger --stack-name sam-app --tail
```

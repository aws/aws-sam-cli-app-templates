...
```bash
{{cookiecutter.project_name}}$ sam local invoke helloFromLambdaFunction --no-event
```
...
```bash
{{cookiecutter.project_name}}$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```

...
```bash
{{ cookiecutter.project_name }}$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```
...
```bash
{{ cookiecutter.project_name }}$ sam logs -n NetCodeWebAPIServerless --stack-name {{ cookiecutter.project_name }} --tail
```

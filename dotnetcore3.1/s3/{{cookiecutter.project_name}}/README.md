...
```bash
{{ cookiecutter.project_name }}$ sam local invoke S3JsonLoggerFunction --event events/event.json
```
...
```bash
{{ cookiecutter.project_name }}$ sam logs -n S3JsonLoggerFunction --stack-name {{ cookiecutter.project_name }} --tail
```

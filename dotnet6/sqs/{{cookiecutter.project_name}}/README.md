...
```bash
{{cookiecutter.project_name}}$ sam local invoke SQSPayloadLogger --event events/event-sqs.json
```
...
Resources:
  ...
  SQSPayloadLogger:
    ...
    Properties:
      CodeUri: ./src/SQSEvent/
      Handler: SQSEvent::SQSEvent.Function::FunctionHandler
      Runtime: dotnet6
      MemorySize: 256
      DeadLetterQueue:
        Type: SQS
...
```bash
{{cookiecutter.project_name}}$ sam logs -n SQSPayloadLogger --stack-name sam-app --tail
```

...
class TestEC2Event(TestCase):
    """
    This integration test will create an EC2 and verify the lambda function is invoked.
    ...
    def setUpClass(cls) -> None:
        ...
        function_resources = [
            ...
            if resource["LogicalResourceId"] == "HelloWorldFunction"
        ]
    ...
    def test_ec2_event(self):
        ...
        while retries >= 0:
            ...
            match_events = [
                event for event in events if self.instance_id in event["message"]
            ]
            if match_events:
                return
            else:
                logging.info(
                    f"Cannot find matching events containing instance id {self.instance_id}, waiting"
                )
                retries -= 1
                sleep(5)

        self.fail(
            f"Cannot find matching events containing instance id {self.instance_id} after 5 retries"
        )
